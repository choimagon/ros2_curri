...
# source paths are relative to ~/agv_ws/src.  The files are read directly into
...
MODULES = [
    ...
    },
    {
        ...
        "goal": "~/agv_ws를 만들고 Python ament_python과 C++ ament_cmake 패키지를 빌드한다.",
        "completion": "build/install/log와 두 언어의 실행 파일을 source 뒤에 확인한다.",
        "start": "M01을 끝낸 터미널. ~/agv_ws가 없거나, 기존 폴더를 별도로 백업한 상태.",
        ...
        "errors": [("패키지를 찾을 수 없음", "~/agv_ws에서 build했는지와 install/setup.bash source를 확인한다."), ("중복 패키지 오류", "src 안에 같은 package name이 두 개 없는지 확인한다.")],
    ...
    },
    {
        ...
        "flow": ["base_link\nbox", "wheel\ncylinder", "sensor\nfixed joint"],
        "files": ["agv_description/urdf/agv.urdf.xacro", "agv_description/urdf/common.xacro", "agv_description/urdf/wheels.xacro", "agv_description/urdf/sensors.xacro", "agv_description/launch/display.launch.py"],
        "actions": [("URDF/Xacro를 펼쳐 검사한다", "Xacro의 macro와 property가 유효 URDF가 되는지 먼저 확인한다.", "cd ~/agv_ws\nxacro src/agv_description/urdf/agv.urdf.xacro > /tmp/agv.urdf\ncheck_urdf /tmp/agv.urdf"), ("RViz로 표시한다", "RobotModel과 TF를 같은 기준 frame에서 함께 확인한다.", "source install/setup.bash\nros2 launch agv_description display.launch.py")],
        "run": "cd ~/agv_ws && source install/setup.bash\nros2 launch agv_description display.launch.py",
        "validate": "xacro ~/agv_ws/src/agv_description/urdf/agv.urdf.xacro > /tmp/agv.urdf\ncheck_urdf /tmp/agv.urdf",
        "capture": "xacro src/agv_description/urdf/agv.urdf.xacro > /tmp/m05.urdf && check_urdf /tmp/m05.urdf",
    ...
    },
    {
        ...
        "files": ["agv_description/urdf/agv.urdf.xacro", "agv_description/urdf/common.xacro", "agv_description/urdf/wheels.xacro", "agv_description/urdf/sensors.xacro"],
        "actions": [("include 관계를 만든다", "공통 관성·wheel·sensor를 파일별 책임으로 분리한다.", "cd ~/agv_ws/src/agv_description/urdf\nmkdir -p .\nxacro agv.urdf.xacro > /tmp/agv_from_xacro.urdf"), ("property 변경을 검증한다", "하나의 수치가 모든 macro 호출에 같은 값으로 전달되어야 한다.", "check_urdf /tmp/agv_from_xacro.urdf\n# wheel_radius 값을 바꾼 뒤 같은 두 명령을 다시 실행")],
        "run": "xacro ~/agv_ws/src/agv_description/urdf/agv.urdf.xacro > /tmp/agv.urdf\ncheck_urdf /tmp/agv.urdf",
        "validate": "xacro ~/agv_ws/src/agv_description/urdf/agv.urdf.xacro > /tmp/agv.urdf\nrg -n 'wheel_radius|left_wheel_joint|right_wheel_joint' /tmp/agv.urdf",
        "capture": "xacro src/agv_description/urdf/agv.urdf.xacro > /tmp/m06.urdf && check_urdf /tmp/m06.urdf",
    ...
    },
    {
        ...
        "flow": ["visual\n보이는 모양", "collision\n접촉", "inertial\n질량/관성", "friction\n접지"],
        "files": ["agv_gazebo/models/agv/model.sdf"],
        "actions": [("SDF 물리 모델을 검사한다", "visual이 보인다고 collision과 inertia가 자동으로 생기지는 않는다.", "cd ~/agv_ws\ngz sdf -k src/agv_gazebo/models/agv/model.sdf"), ("값을 한 번에 하나씩 바꾼다", "증상 원인을 찾으려면 mass→inertia→collision→friction 순서를 지킨다.", "rg -n '<mass>|<inertia>|<collision>|<friction>' src/agv_gazebo/models/agv/model.sdf")],
        "run": "cd ~/agv_ws && source install/setup.bash\nros2 launch agv_gazebo gazebo.launch.py",
        "validate": "gz sdf -k ~/agv_ws/src/agv_gazebo/models/agv/model.sdf",
        "capture": "gz sdf -k src/agv_gazebo/models/agv/model.sdf; rg -n '<mass>|<friction>|left_wheel_joint' src/agv_gazebo/models/agv/model.sdf",
    ...
    },
    {
        ...
        "files": ["agv_gazebo/worlds/warehouse.sdf", "agv_gazebo/models/agv/model.config", "agv_gazebo/launch/gazebo.launch.py"],
        "actions": [("model 파일을 먼저 검증한다", "CLI의 `gz sdf -k`는 standalone World에서 model:// URI를 해석하지 못할 수 있다. AGV model 자체를 먼저 검사한다.", "cd ~/agv_ws\ngz sdf -k src/agv_gazebo/models/agv/model.sdf\nrg -n 'model://agv|<world name=' src/agv_gazebo/worlds/warehouse.sdf"), ("launch로 World와 bridge를 시작한다", "launch가 Gazebo resource path와 World 실행을 한 번에 준비하므로 World는 이 명령으로 검증한다.", "source install/setup.bash\nros2 launch agv_gazebo gazebo.launch.py")],
        "run": "cd ~/agv_ws && source install/setup.bash\nros2 launch agv_gazebo gazebo.launch.py",
        "validate": "gz sdf -k ~/agv_ws/src/agv_gazebo/models/agv/model.sdf\nrg -n 'model://agv|<world name=' ~/agv_ws/src/agv_gazebo/worlds/warehouse.sdf\n# 이어서 ros2 launch agv_gazebo gazebo.launch.py로 World를 연다.",
        "capture": "gz sdf -k src/agv_gazebo/models/agv/model.sdf; printf '\\n--- World model URI ---\\n'; rg -n 'model://agv|<world name=' src/agv_gazebo/worlds/warehouse.sdf",
        "visual": "blocks/B_robot_build/captures/gazebo_agv_actual.png", "visual_caption": "실제 Gazebo Sim에서 warehouse World에 spawn된 AGV",
    ...
    },
    {
        ...
        "files": [],
        "actions": [("RViz를 설정 파일로 연다", "같은 display 설정을 다음 센서 모듈에서도 재사용한다.", "source ~/agv_ws/install/setup.bash\nrviz2 -d ~/agv_ws/src/agv_description/rviz/agv.rviz"), ("RobotModel과 TF를 먼저 확인한다", "모델과 frame이 맞아야 scan·image 위치도 맞는다.", "Fixed Frame: odom 또는 base_link\nAdd → RobotModel → Description Topic: /robot_description\nAdd → TF"), ("센서 display를 추가한다", "각 display는 topic과 message frame_id를 TF로 변환한다.", "Add → LaserScan → Topic: /scan\nAdd → Odometry → Topic: /odom\nFile → Save Config As → agv.rviz")],
        ...
        "visual": "blocks/C_drive_visualization/captures/rviz_agv_sensors_actual.png", "visual_caption": "실제 RViz2: AGV body·wheel·camera·LiDAR·IMU frame",
    ...
    },
    {
        ...
        "goal": "controller_manager·joint_state_broadcaster·diff_drive_controller 설정의 책임을 이해한다.",
        "completion": "wheel joint 이름과 controller YAML의 좌·우 목록·반지름·wheel separation을 일치시킨다.",
        ...
        "files": ["agv_control/config/controllers.yaml", "agv_description/urdf/agv.urdf.xacro"],
        "actions": [("controller YAML을 작성한다", "controller type과 wheel parameter는 URDF/SDF joint 이름과 같아야 한다.", "nano ~/agv_ws/src/agv_control/config/controllers.yaml\nros2 pkg prefix gz_ros2_control"), ("plugin 전환 전 기준선을 확인한다", "이 저장소의 기본 실행은 M09 native DiffDrive다. gz_ros2_control로 바꾸기 전 joint·config를 먼저 검증한다.", "ros2 pkg prefix controller_manager\nros2 pkg prefix diff_drive_controller\nros2 pkg prefix gz_ros2_control")],
        "run": "source ~/agv_ws/install/setup.bash\nros2 pkg prefix gz_ros2_control\nros2 pkg prefix controller_manager",
        "validate": "rg -n 'left_wheel_joint|right_wheel_joint|wheel_separation|wheel_radius' ~/agv_ws/src/agv_control/config/controllers.yaml\nrg -n 'left_wheel_joint|right_wheel_joint' ~/agv_ws/src/agv_gazebo/models/agv/model.sdf",
    ...
    },
    {
        ...
        "actions": [("YAML 책임을 분리한다", "robot·sensor·vision·mission·controller 값은 실행 코드와 분리한다.", "find ~/agv_ws/src -path '*/config/*.yaml' -type f | sort\nros2 param get /mission_manager stop_distance"), ("한 launch로 실행한다", "launch는 Gazebo, TF, processing, mission, safety의 시작 순서를 한 곳에서 관리한다.", "source ~/agv_ws/install/setup.bash\nros2 launch agv_bringup agv_sim.launch.py"), ("주행 데이터를 기록한다", "bag은 simulator 재실행 없이 processing 결과를 비교할 입력 데이터다.", "ros2 bag record -o ~/agv_bag_01 \\\n  /scan /imu/data /odom /camera/image_raw\n# Ctrl-C 뒤: ros2 bag info ~/agv_bag_01")],
...
def make_terminal_capture(module: dict, text: str, target: Path) -> None:
    command = module["capture"].replace(str(ROOT), "~/agv_ws")
    lines = [f"$ cd ~/agv_ws", f"$ {command}", ""] + text.splitlines()
    wrapped = []
    for line in lines:
...
def status(slide, module: dict, step: str, folder: str = "~/agv_ws", new_terminal: str = "현재 터미널") -> None:
...
def add_action_slide(slide, module: dict, step_number: int, total_steps: int, action: tuple[str, str, str]) -> None:
    ...
    status(slide, module, f"{step_number}/{total_steps}", "~/agv_ws", "새 터미널" if "새 터미널" in command else "현재 터미널")
...
def add_errors_slide(slide, module: dict) -> None:
    ...
    status(slide, module, "오류 대응", "~/agv_ws")
...
def add_checkpoint_slide(slide, module: dict, artifact_dir: Path) -> None:
    ...
    status(slide, module, "체크포인트", "~/agv_ws")
...
def add_extra_slide(slide, module: dict, heading: str, bullets: list[str]) -> None:
    ...
    status(slide, module, "통합 보강", "~/agv_ws")
...
def build_deck(module: dict) -> tuple[Path, str]:
    ...
    status(slide, module, "시작", "~/agv_ws")
    panel = box(slide, 0.78, 1.95, 12.0, 3.75, fill=LIGHT, line=RGBColor(215, 225, 235))
    add_bullets(slide, 1.0, 2.20, 11.4, 3.10, [f"시작 상태: {module['start']}", f"이전 Complete: {module['previous']}", "현재 폴더: ~/agv_ws", "새 터미널마다: source /opt/ros/jazzy/setup.bash → source install/setup.bash", "파일 경로는 PPT와 Complete 폴더에서 같은 이름으로 유지"], 19)
    text_box(slide, 0.98, 6.05, 11.5, 0.35, "확인: pwd가 ~/agv_ws이고, 이전 모듈의 완료 조건을 한 문장으로 설명할 수 있으면 시작합니다.", 16, GREEN, True, align=PP_ALIGN.CENTER)
    add_notes(slide, "교육생의 시작 상태를 통일한다.", command="pwd; source /opt/ros/jazzy/setup.bash; source ~/agv_ws/install/setup.bash", check="현재 폴더와 source 상태를 모두 확인한다.")
    ...
    tree_lines = ["~/agv_ws/src/"] + ["└── " + value for value in files]
    ...
    status(slide, module, "미니 실습", "~/agv_ws")
...
def make_artifacts(module: dict, validation_log: str) -> None:
    ...
    (starter / "README.md").write_text(
        ...
        "- 실제 작업 workspace는 `~/agv_ws`이고, source 원본은 이 저장소의 `agv_ws/src/`이다.\n",
        encoding="utf-8")
    manifest = []
    for relative in module["files"]:
        source = SRC / relative
