# ROS 2 기반 AGV End-to-End 개발 커리큘럼 — PPT 배포 구조

M01~M22를 각각 독립 PPT로 배포한다. 실제 수업에서는 각 모듈 폴더의 `MXX_*.pptx`를 순서대로 사용한다. 각 Block 루트의 `Block_*_M시리즈_통합_따라하기.pptx`는 해당 M 시리즈를 순서대로 합친 보기용 자료다. 저장소 루트의 `ROS_2_기반_AGV_End-to-End_개발_커리큘럼_전체_통합_따라하기.pptx`는 Block A~F 전체와 M01~M22를 순서대로 합친 전체 강의·복습 자료다. 로컬 `.docx` 원본 가이드는 Git에서 제외하며, 별도의 ‘실습 결과 명령어’ 요약 PPT는 배포하지 않는다.
...
실행 슬라이드의 검은색 terminal 블록에는 **복사해서 실행할 명령만** 넣는다. `왜 하는가`, 각 명령의 역할, 정상 확인 기준은 반드시 블록 밖에 둔다. 두 terminal이 필요한 경우에는 `터미널 1`과 `터미널 2`를 별도 검은 블록으로 나누고, 두 블록 모두에 필요한 `source` 명령을 넣는다. 줄바꿈·들여쓰기·백슬래시(`\`)는 실제 shell 명령 그대로 보존하며, terminal 구분용 `#` 주석과 설명 문장은 복사 블록에서 제외한다.

URDF/Xacro·SDF·Gazebo·RViz 관련 모듈은 실제 화면을 구현 전 목표 화면과 실행 직후 결과 화면에 모두 넣는다. 모델 형상은 RViz RobotModel·TF 화면으로, 물리/World/구동은 Gazebo Sim 화면으로, 센서는 RViz·Camera·terminal 화면으로 연결해 코드·모델·결과를 한 페이지 흐름에서 대조한다.
