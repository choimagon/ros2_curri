...
7. 파일별 코드 블록 입력·저장
8. build → source → run과 실제 결과 확인
9. 객관적 검증 명령, 대표 오류, 미니 실습
10. 막혔을 때만 Complete와 diff·backup·복구
11. 내 workspace checkpoint와 공식 참고 자료

코드 슬라이드는 `~/ros2_curri/my_agv_ws/src/...`의 빈 파일을 먼저 열고, 해당 파일의 코드 블록을 위에서 아래로 직접 입력·저장하게 한다. 저장소의 `agv_ws/`는 참고 구현이며 학습자가 직접 고치지 않는다. 길면 `입력 1/N`처럼 이어지며 `...`로 생략하지 않는다. `complete/`는 먼저 복사하는 정답지가 아니라, 오류가 해결되지 않을 때 `diff`로 비교하고 backup 뒤 복구하는 안전망이다. 제목은 30 pt 이상, 본문은 18 pt 이상, 코드/명령은 14 pt, 하단 주석은 10 pt 이상으로 생성한다. 발표자 노트에는 설명·실행·확인·멈춤·오류를 넣는다.
